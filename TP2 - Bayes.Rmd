...
d_propuesta <- function(x, mean) dbeta(x, shape1 = mean * concentracion3[i], shape2 = (1-mean) * concentracion3[i])
r_propuesta <- function(x) rbeta(1, shape1 = x * concentracion3[i], shape2 = (1-x) * concentracion3[i])
#Donde x hace referencia a mu
n3 <- 5000
p_inicial <- rbeta(1,shape1=2,shape2=2)
funcion3 <- sample_mh(d_objetivo, r_propuesta, d_propuesta, p_inicial, n3)
...
plot_hist <- function(x, d_objetivo) {
  x_seq <- seq(min(x), max(x), length.out = n_3)
  df_hist <- data.frame(x = x)
  df_line <- data.frame(x = x_seq, y = d_objetivo(x_seq))
  
  ggplot(df_hist)+
    geom_histogram(aes(x = x, y =after_stat(density))) +
    geom_line(aes(x = x, y = y), data = df_line, color = "red")
}
grafico3_5 <- plot_hist(tabla3$muestra[1:5000],d_objetivo) +
  labs (title = "k=4", x = "Muestras", y = "Densidad")
grafico3_6 <- plot_hist(tabla3$muestra[5001:10000],d_objetivo) +
  labs (title = "k=10", x = "Muestras", y = "Densidad")
grafico3_7 <- plot_hist(tabla3$muestra[10001:15000],d_objetivo) +
...
```{r}
#Punto 4
muestra1 <- tabla_3$muestra[1:5000]
muestra2 <- tabla_3$muestra[5001:10000]
muestra3 <- tabla_3$muestra[10001:15000]

#Calculo de la media y los percentiles 5 y 95 para X:
medias <- c(mean(muestra1), mean(muestra2), mean(muestra3))
percentiles <- data.frame(
  Muestra = c("1", "2", "3"),
  Percentiles = c(quantile(muestra1, prob = c(0.05)),
                  quantile(muestra2, prob = c(0.05)),
                  quantile(muestra3, prob = c(0.05))
                  ),
  
)

#Cálculo de la media y los percentiles 5 y 95 para logit(X):
l_m1 <- log((muestra1/1-muestra1))
l_m2 <- log((muestra2/1-muestra2))
l_m3 <- log((muestra3/1-muestra3))
