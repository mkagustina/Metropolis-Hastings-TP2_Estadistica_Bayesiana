...
B <-  function(muestras, M, S) {
  ...
  media_g_mu1 <- mean(data_mu1)
  media_g_mu2 <- mean(data_mu2)
  media_cadena_mu1 <- mean(apply(data_mu1, 2, mean))
  media_cadena_mu2 <- mean(apply(data_mu2, 2, mean))
  
  
  b_mu1 <- (S/(M-1))* sum((media_cadena_mu1-media_g_mu1)^2)
  b_mu2 <- (S/(M-1))* sum((media_cadena_mu2-media_g_mu2)^2)
  #S = cantidad observaciones
  #M = cantidad muestras
  return(list(b_mu1 = b_mu1, b_mu2 = b_mu2))
}
...
r_hat <- function(x) {
  ...
  b_mu1 <- B$b_mu1
  b_mu2 <- B$b_mu2
  r_hat_mu1 <- sqrt(((S/(S-1))*W + (1/S)*b_mu1)/W)
  r_hat_mu2 <- sqrt(((S/(S-1))*W + (1/S)*b_mu2)/W)
  return (list (r_hat_mu1 = r_hat_mu1, r_hat_mu2 = r_hat_mu2))
}
...
n_eff7_2 <- n_eff(muestra7[,2])
 return(list(grafico = grafico7_1, n1 = n_eff7_1, n2 = n_eff7_2, r = r_sombrero,
    ...
    j <- c(1, 3, 5)
    for(i in j) {
      p_inicial7 <- rmvnorm(1, mean = c(0.4,0.75), sigma = diag(2)) 
      funcion7 <- sample_mh(d_objetivo7,r_propuesta7,d_propuesta7,p_inicial7,n7)
      muestras[,i] <- funcion7$muestras[,1]
      muestras[,i+1] <- funcion7$muestras[,2]
      
    }

grafico7_1 <- plot_trace(muestras[,1],muestras[,2])
r_sombrero <- r_hat(muestras)
r_sombrero_mu1 <- r_sombrero$r_hat_mu1
r_sombrero_mu2 <- r_sombrero$r_hat_mu2
n_eff7_mu1 <- n_eff(muestras[,1])
n_eff7_mu2 <- n_eff(muestras[,2])
 return(list(grafico = grafico7_1, n1 = n_eff7_mu1, n2 = n_eff7_mu2, r_mu1 = r_hat_mu1, r_mu2 = r_hat_mu2, muestra7 = muestra7))
...
p_estrella <- function(x, y, a, b){
  exp(-((((a - x)^2) + b * ((y - x)^2))^2))
}

d_objetivo9 <- function(x, y) p_estrella(x, y, a, b)

d_propuesta9 <- function()
r_propuesta9 <- 
```
